...
export const obtenerPaymentIntent = async (req, res) => {
    try {
        ...
        // Extraer el ID real del clientSecret
        ...
        // Extraer información útil del PaymentIntent
        const paymentInfo = {
            id: paymentIntent.id,
        ...
        // Información adicional de MSI si existe
        ...
        // Si el pago fue exitoso, incluir información del cargo
        if (paymentIntent.status === 'succeeded' && paymentIntent.charges?.data?.[0]) {
            ...
            if (charge.payment_method_details?.card?.installments) {
                ...
                paymentInfo.charge.msi = {
                    plan: chargeInstallments.plan,
                    count: chargeInstallments.count
                };
            }
        }
        ...
        if (error.code === 'resource_missing') {
            ...
            errorMessage = 'Formato de clientSecret inválido';
        }
